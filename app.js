...
(function(){
  ...
  // 品类是器物站的第一浏览轴（器物八成佚名，艺术家轴在这里没有意义）
  ...
  let eraVals = [], mediumVals = [], countryVals = [], catVals = [];
  ...
  function rebuildSelects(){
    const e=eraFilter.value, m=mediumFilter.value, c=countryFilter.value, k=catFilter?catFilter.value:"";
    if(catFilter) buildSelect(catFilter, "cat", catVals, "all_cats");
    ...
    eraFilter.value=e; mediumFilter.value=m; countryFilter.value=c; if(catFilter) catFilter.value=k;
  }

  // 统计（值在 computeDerived 后更新）
  function updateStats(){ $("era-count").textContent = eraVals.length; $("artist-count").textContent = catVals.length; }
  ...
  function computeDerived(){
    buildTrMaps();
    eraVals = uniq("era"); mediumVals = uniq("medium"); countryVals = uniq("country"); catVals = uniq("cat");
  ...
  function applyFilters(){
    ...
    const fe = eraFilter.value, fm = mediumFilter.value, fc = countryFilter.value, fk = catFilter?catFilter.value:"";
    filtered = DATA.filter(d => {
      ...
      if(q){
        if(d._h === undefined) d._h = (d.title+" "+d.artist+" "+d.year+" "+d.era+" "+d.medium+" "+d.location+" "+(d.country||"")+" "+
          (d.cat||"")+" "+(d.cat_en||"")+" "+(d.fmt||"")+" "+(d.fmt_en||"")+" "+(d.tg?d.tg.join(" "):"")+" "+(d.tg_en?d.tg_en.join(" "):"")+" "+
  ...
  function applyLangChrome(){
    ...
    { const ab=$("artist-btn"); if(ab) ab.textContent = T("by_artist"); }

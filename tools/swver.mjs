...
//
// 病灶：sw.js 里 SHELL = "art1001-shell-v68" 一直靠人手改。改完 CSS/JS 忘了 bump，
...
for (const f of files.sort()) {
  ...
  const body = f === 'sw.js' ? readFileSync(p(f), 'utf8').replace(/art1001-shell-[a-z0-9]+/g, '') : readFileSync(p(f));
  h.update(f).update(body); n++;
}
const ver = h.digest('hex').slice(0, 10);
const want = `art1001-shell-${ver}`;
